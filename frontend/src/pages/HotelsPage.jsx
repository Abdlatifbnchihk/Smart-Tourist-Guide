...
export default function HotelsPage() {
  ...
  const filtered = searchQuery
    ? hotels.filter((h) =>
        ...
        h.city?.name?.toLowerCase().includes(searchQuery.toLowerCase())
      )
    : hotels
  ...
  return (
    <div className="pt-20 pb-12 px-4 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-800 mb-2">Hotels</h1>
          <p className="text-slate-600">Find the best places to stay across Morocco</p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            <p className="text-slate-600">No hotels found{searchQuery && ` for "${searchQuery}"`}</p>
          </div>
        ) : (
