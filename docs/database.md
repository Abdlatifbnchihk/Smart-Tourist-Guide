...
The database is designed for a **multi-role tourism platform** covering four core domains:
...
All primary keys are `BIGINT` auto-increment.
All tables use Laravel's standard `created_at` / `updated_at` timestamps.

**Key design decisions:**
- Role-based access is handled via an ENUM on the `users` table (no separate `roles` table).
- Bookings are stored in a single unified `bookings` table with nullable FKs for `room_id` and `driver_id`.
- Reviews and favorites use explicit FK columns per entity (not polymorphic relationships).
- Account moderation is managed via a `status` ENUM on the `users` table.
